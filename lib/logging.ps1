Function Log-Start{
    ...
    Process{
        Try {
            ...
            Add-Content -Path $sFullPath -Value "pFolder - Started processing at [$([DateTime]::Now)]."
            Add-Content -Path $sFullPath -Value "Running pFolder version [$ScriptVersion]"
            Add-Content -Path $sFullPath -Value "***************************************************************************************************"
            Add-Content -Path $sFullPath -Value ""
...
Function Log-Error{
    ...
    Process{
		...
		#If $ExitGracefully = True then run Log-Finish, send Email and exit script
...
Function Log-Email{
    ...
    Process{
        Try{
            # Clear HTMLTemplate
            replaceFileString -Pattern '{.+}' -Replacement "" -Path $mailTemplateTmp -Overwrite

			# $sBody = (Get-Content $LogPath | out-string)
            $sBody = Get-Content $mailTemplateTmp
            ...
            $message.Body = $sBody
		    $oSmtp = new-object Net.Mail.SmtpClient($appSettings["server"])
            $oSmtp.Credentials = New-Object System.Net.NetworkCredential($appSettings["username"], $appSettings["password"]);
		    $oSmtp.Send($message)

            # Remove temporary template after email is sent
            Remove-Item $mailTemplateTmp
		}
